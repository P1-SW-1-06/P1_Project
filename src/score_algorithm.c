...
temp_score **score_algorithm(int** distance_array, people_data *final_user, int nr_of_cities, int number_of_people,
    ...
    for (int person_count = 0; person_count < number_of_people; ++person_count) {
        ...
        for (int city_count = 0; city_count < nr_of_cities; ++city_count) {
            ...
            sum_of_cost_scores(person_count, score_array, final_user, city_count, max_transport_cost, yearly_max_housing_cost);
        }
    }
    for (int person_count = 0; person_count < number_of_people; ++person_count) {
        for (int city_count = 0; city_count < nr_of_cities; ++city_count) {
            ...
            else score_array[person_count][city_count].sum_of_scores.score_bus =
            ...
            else score_array[person_count][city_count].sum_of_scores.score_car =
...
void env_score(temp_score **scorearray,int person_number, int *distance, people_data *final_user, transport_vehicle_info *transport_data, int city) {
    ...
    double max_result_co2 = (final_user[person_number].max_time / (1 / transport_data[car].speed) * transport_data[car].co2 * 189 *
                             2);
    if (final_user[person_number].exclusion.include_bus == 1) {
        scorearray[person_number][city].environment.score_bus = (distance[city] * transport_data[bus].co2 * 189 * 2) / max_result_co2 *
                                                final_user[person_number].preference_environment;
    }
    if (final_user[person_number].exclusion.include_car == 1) {
        scorearray[person_number][city].environment.score_car = (distance[city] * transport_data[car].co2 * 189 * 2) / max_result_co2 *
                                                final_user[person_number].preference_environment;
    }
    if (final_user[person_number].exclusion.include_bike == 1) {
...
void transport_cost_score(temp_score **scorearray,int person_number, int *distance, transport_vehicle_info *transport_data,
                     int city) {
