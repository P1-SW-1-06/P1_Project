...
void print_user_guide(){
    printf("User guide:\n"
           ...
           "and may therefore not be the the best overall solution\nfor your situation.\n"
           "The output is only based on commuting preferences, and\nare therefore up for interpretation by the user.\n\n"
...
void print_final_score(int number_of_people, int number_of_available_cities, shared_score *final_combined_city_score,
    ...
    printf("Rank\t City\t\t Score\n");
    printf("------------------------------\n");
    for (int i = 0; i < number_of_available_cities; ++i) {
        printf("%d\t %-14s\t %.1lf\n",i+1, final_combined_city_score[i].city_name,
